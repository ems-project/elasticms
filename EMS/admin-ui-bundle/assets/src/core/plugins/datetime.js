    ...
    loadPicker(target, query, options) {
        ...
        for (let i = 0; i < pickers.length; i++) {
            ...
            if (pickers[i].dataset.weekStart) {
                options.localization.startOfTheWeek =
                    pickers[i].dataset.weekStart;
            }
            ...
            if (pickers[i].dataset.daysOfWeekDisabled) {
                options.restrictions.daysOfWeekDisabled = JSON.parse(
                    pickers[i].dataset.daysOfWeekDisabled,
                );
            }
            ...
            if (pickers[i].dataset.dateDisabledHours) {
                options.restrictions.disabledHours = JSON.parse(
                    pickers[i].dataset.dateDisabledHours,
                );
            }
            ...
            if (pickers[i].dataset.displayOption) {
                const displayOptions = JSON.parse(
                    pickers[i].dataset.displayOption,
                );
                if (undefined !== displayOptions.locale.firstDay) {
                    options.localization.startOfTheWeek =
                        displayOptions.locale.firstDay;
                }
                ...
                if (undefined !== displayOptions.timePicker) {
                    options.display.components.clock =
                        displayOptions.timePicker;
                }
                if (
                    undefined !== displayOptions.timePicker24Hour &&
                    displayOptions.timePicker24Hour
                ) {
                    options.localization.hourCycle = "h23";
                } else {
                    options.localization.hourCycle = "h12";
                ...
                if (
                    undefined !== displayOptions.showWeekNumbers &&
