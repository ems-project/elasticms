...
final class FakeProjectBuilder extends Command
{
    /**
     * @var array<string>
     */
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $destination = $this->resolveDestination((string) $input->getArgument('destination'), $projectDir);
        $force = (bool) $input->getOption('force');

        try {
            $composer = $this->loadComposer(\sprintf('%s/composer.json', $projectDir));
            $this->assertDestination($destination, $force);
            $writtenFiles = $this->writeFakeProject($destination, $composer, $force);
        ...
        $io->success(\sprintf('Fake Symfony project generated in %s', $destination));
        $io->listing([
     ...
     */
    private function writeFakeProject(string $destination, array $composer, bool $force): int
    {
        $count = 0;
        foreach ($this->files($composer) as $relativePath => $file) {
            $target = $destination.'/'.$relativePath;
            $this->writeFile($target, $file['contents'], $force);
