...
export default class Button {
    ...
    addDisabledButtonTreatListeners(target) {
        ...
        if (treat) {
            treat.addEventListener("change", function () {
                ...
                form.elements.treat_notifications_accept.disabled =
                    isDisabledAccept;
                form.elements.treat_notifications_reject.disabled =
                    !isDisabledAccept;
            });
        }
    }
    ...
    addPostButtonListeners(target) {
        ...
        for (let i = 0; i < buttons.length; ++i) {
            buttons[i].addEventListener("click", function (e) {
                ...
                if (
                    Object.hasOwn(postSettings, "value") &&
                    Object.hasOwn(postSettings, "name")
                ) {
                    const inputField = document.createElement("INPUT");
                    inputField.style.display = "none";
                    inputField.type = "TEXT";
    ...
    addToggleButtonListeners(target) {
        const buttons = target.querySelectorAll(
            "[data-bs-toggle-contain].toggle-button",
        );
        for (let i = 0; i < buttons.length; ++i) {
