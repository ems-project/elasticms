...
class Cropper {
    constructor(container) {
        ...
        this.backgroundColor = container.querySelector(
            ".ems-cropper-background-color",
        );
        this.data = null;
        ...
        container
            .querySelector(".ems-cropper-zoom-in")
            .addEventListener("click", () => self.zoom(0.1));
        container
            .querySelector(".ems-cropper-zoom-reset")
            .addEventListener("click", () => self.reset());
    ...
    ready() {
        ...
        if (this.data.width > canvasData.naturalWidth) {
            canvasData.width -= Math.ceil(
                (this.data.width - canvasData.naturalWidth) * scaleX,
            );
        }
        if (this.data.height > canvasData.naturalHeight) {
            canvasData.height -= Math.ceil(
                (this.data.height - canvasData.naturalHeight) * scaleY,
            );
        }
    ...
    setBackgroundColor() {
        if (
            ...
            this.backgroundColor.value === "#000000"
        ) {
            ...
            this.container.querySelector(
                ".cropper-crop-box",
            ).style.backgroundColor = this.backgroundColor.value;
        }
    }
}
