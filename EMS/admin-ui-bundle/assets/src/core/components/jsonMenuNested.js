...
export default class JsonMenuNested {
    ...
    getStructureJson(includeRoot = false) {
        const makeChildren = (element) => {
            ...
            if (childList) {
                childList
                    .querySelectorAll(":scope > li.json-menu-nested-item")
                    .forEach((li) => {
    ...
    selectItem(itemId, scroll = false) {
        ...
        this.target
            .querySelectorAll("li.json-menu-nested-item")
            .forEach((li) => {
    ...
    _parseAttributes() {
        if (this.target.hasAttribute("data-hidden-field-id")) {
            this.hiddenField = document.getElementById(
                this.target.dataset.hiddenFieldId,
            );
        }
    ...
    _relocate() {
        ...
        this.target.querySelectorAll("ol").forEach((ol) => {
            ...
            if (
                !ol.classList.contains("collapse") &&
                !ol.classList.contains("json-menu-nested-root")
            ) {
        ...
        if (Object.hasOwn(this, "hiddenField") && this.hiddenField !== null) {
            if (
                this.hiddenField.classList.contains(
                    "json-menu-nested-silent-publish",
                )
            ) {
                this.hiddenField.value = this.getStructureJson(true);
                this.hiddenField.dispatchEvent(
                    new CustomEvent("silentPublish"),
                );
            } else {
    ...
    _getElementItem(itemId) {
        return this.target.parentElement.querySelector(
            `[data-item-id="${itemId}"]`,
        );
    }
    ...
    _getCopy() {
        if (Object.hasOwn(localStorage, this.copyName)) {
            ...
            return loopJson(json, (key, value) =>
                key === "id" && value !== "_root" ? v4() : value,
            );
        }
    ...
    _buttonItemAdd(element) {
        element
            .querySelectorAll(".btn-json-menu-nested-add")
            .forEach((btnAdd) => {
                btnAdd.onclick = (e) => {
                    e.preventDefault();
    ...
    _buttonItemEdit(element) {
        element
            .querySelectorAll(".btn-json-menu-nested-edit")
            .forEach((btnEdit) => {
                btnEdit.onclick = (e) => {
                    e.preventDefault();
                    ...
                    const item = JSON.parse(
                        this._getElementItem(itemId).dataset.item,
                    );
                    ...
                    const callback = (json) => {
                        ...
                        if (ol) {
                            this._getElementItem(itemId).insertAdjacentHTML(
                                "beforeend",
                                ol.outerHTML,
                            );
                        }
                        ...
                        this._addEventListeners(
                            this._getElementItem(itemId).parentNode,
                        );
                    };
    ...
    _buttonItemDelete(element) {
        element
            .querySelectorAll(".btn-json-menu-nested-delete")
            .forEach((btnDelete) => {
                btnDelete.onclick = (e) => {
                    e.preventDefault();
    ...
    _buttonItemCopyAll(element) {
        const btnCopyAll = element.querySelector(
            ".btn-json-menu-nested-copy-all",
        );
        if (btnCopyAll === null) {
            return;
        }
    ...
    _buttonItemCopy(element) {
        element
            .querySelectorAll(".btn-json-menu-nested-copy")
            .forEach((btnCopy) => {
                btnCopy.onclick = (e) => {
                    e.preventDefault();
                    ...
                    const liToObject = (li) => {
                        ...
                        const children = [];
                        const childList = this._getElementItemList(
                            li.dataset.itemId,
                        );
                        if (childList) {
                            childList
    ...
    _buttonItemPaste(element) {
        element
            .querySelectorAll(".btn-json-menu-nested-paste")
            .forEach((btnPaste) => {
                btnPaste.onclick = (e) => {
                    e.preventDefault();
    ...
    _buttonItemPreview(element) {
        element
            .querySelectorAll(".btn-json-menu-nested-preview")
            .forEach((btnPreview) => {
    ...
    _refreshPasteButtons() {
        ...
        document
            ...
            .forEach((btnPaste) => {
                ...
                if (
                    (node !== undefined && node.addNodes.includes(copyType)) ||
                    allow === copyType
                ) {
