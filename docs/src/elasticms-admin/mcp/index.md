...
project-specific MCP tools and resources configured in the CMS.

## MCP concepts

ElasticMS uses two MCP concepts for custom CMS integrations:
...
Use a **tool** when the model must execute logic with input parameters. Use a **resource** when the
model needs contextual information it can read by URI.
