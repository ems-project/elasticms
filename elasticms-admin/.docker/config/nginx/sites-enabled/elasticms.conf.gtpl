...
{{ $alias := .Env.ALIAS | strings.TrimSuffix "/" -}}

include conf.d/symfony-script-name.map;
...
{{ if ne $alias "" }}
    ...
    location @alias {
        rewrite ^{{ $alias }}(/.*)?$ /index.php$is_args$args last;
    }
{{- end }}
    ...
    location ~ ^/index\.php(/|$) {
        ...
        fastcgi_param SCRIPT_NAME $symfony_script_name;
