...
class IFrame {
  ...
  _fixHeight(tabEmpty = false) {
    if ($('body').hasClass(CLASS_NAME_FULLSCREEN_MODE)) {
      ...
      $(`${SELECTOR_TAB_EMPTY}, ${SELECTOR_TAB_LOADING}, ${SELECTOR_CONTENT_IFRAME}`).height(
  ...
  static _jQueryInterface(config, name, link, id, reload) {
    if ($(SELECTOR_DATA_TOGGLE).length > 0) {
      ...
      $(this).data(
        DATA_KEY,
        typeof config === 'object' ? config : { link, name, id, reload, ...data }
      )
      if (
      ...
      window.iFrameInstance = new IFrame(
        $(this),
