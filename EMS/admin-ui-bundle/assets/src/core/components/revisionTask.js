...
export default class RevisionTask {
    constructor() {
        ...
        if (this.tasksTab !== null) {
            ...
            this.revisionTaskLoading = this.revisionTasks.querySelector(
                "div#revision-tasks-loading",
            );
            this.revisionTasksContent = this.revisionTasks.querySelector(
                "div#revision-tasks-content",
            );
        ...
        document.addEventListener("click", (e) => {
            ...
            if (e.target.classList.contains("btn-task-change-owner-modal")) {
                ...
                ajaxModal.load(
                    ...
                    (json) => {
                        if (
                            Object.hasOwn(json, "modalSuccess") &&
                            json.modalSuccess === true
                        ) {
    ...
    _addClickListeners() {
        this.tasksTab.addEventListener(
            ...
            (event) => {
                ...
                if (target.classList.contains("btn-task-handle"))
                    this._onClickButtonHandle(target);
                if (target.id === "btn-tasks-reorder")
                    this._onClickButtonTaskReorder(target);
                if (target.id === "btn-tasks-approved")
                    this._onClickButtonTasksApproved(event, target);

                const closestTasksItem = target.closest(".tasks-item");
                if (
                    closestTasksItem ||
                    target.classList.contains("tasks-item")
                ) {
        ...
        document.addEventListener("click", (event) => {
            const target = event.target;
            if (target.id === "btn-task-delete")
                this._onClickButtonTaskDelete(target);
        });
    }
    ...
    _onClickButtonTaskReorder(button) {
        ...
        const btnReorderCancel = this.tasksTab.querySelector(
            "#btn-tasks-reorder-cancel",
        );
        const btnReorderSave = this.tasksTab.querySelector(
            "#btn-tasks-reorder-save",
        );
        ...
        const tasksPlannedList = this.tasksTab.querySelector(
            "ul#revision-tasks-planned-list",
        );
        tasksPlannedList
            .querySelectorAll(".tasks-item")
            .item(0)
            .classList.remove("tasks-item-current");
