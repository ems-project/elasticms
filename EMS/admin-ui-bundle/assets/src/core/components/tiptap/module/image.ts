...
// --- Browse existing uploads (legacy EMS image browser popup) --------------

declare global {
    interface Window {
        emsImageBrowserCallback?: (funcNum: string | number, url: string) => void
    }
}

let funcCounter = 0
const pendingBrowseCallbacks = new Map<number, (url: string) => void>()

function ensureBrowserCallbackBridge(win: Window): void {
    if (typeof win.emsImageBrowserCallback === 'function') return
    win.emsImageBrowserCallback = (funcNum, url) => {
        const n = Number(funcNum)
        const callback = pendingBrowseCallbacks.get(n)
        pendingBrowseCallbacks.delete(n)
        callback?.(url)
    }
}
...
function openImageBrowser(editor: TiptapEditor, onSelect: (url: string) => void): void {
    ...
    const win = editor.docParent.defaultView ?? window
    ensureBrowserCallbackBridge(win)

    const funcNum = ++funcCounter
    pendingBrowseCallbacks.set(funcNum, onSelect)

    const assetPath = editor.docParent.body.dataset.assetPath ?? '/'
    const url =
        `${assetPath}bundles/emscore/js/cke-plugins/imagebrowser/browser/browser.html` +
        `?listUrl=${encodeURIComponent(listUrl)}&CKEditorFuncNum=${funcNum}&langCode=${encodeURIComponent(editor.locale)}`

    win.open(url, 'ems-image-browser', 'width=900,height=650,resizable=yes,scrollbars=yes')
}
