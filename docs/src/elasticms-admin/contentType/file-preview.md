...
In elasticMS admin, when you have a file field, the system tries to generate a thumbnail, if the
file is an image.
...
You can use the `EMSCO_ASSET_CONFIG` environment variable to define mutiple asset configs. Then you
can refer to those config's names in the Display option of any file field configs.
...
With the twig `emsco_asset_path` function you can generate a thumbnail using one of the defined
asset configs:
