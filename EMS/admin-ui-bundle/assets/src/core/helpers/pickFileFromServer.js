import { pickFileModal } from "./ajaxModal";
...
export default class PickFileFromServer {
    ...
    onClick(button) {
        pickFileModal.load(
            { url: button.dataset.href, title: button.textContent, size: "lg" },
            (json, modal) => {
                    ...
                    for (let i = 0; i < linkList.length; i++) {
                        linkList[i].onclick = (event) => {
                            if (
                                ...
                                event.target.parentNode.dataset.json ===
                                    undefined
                            ) {
                                return;
                            }
                            ...
                            row.dispatchEvent(
                                new CustomEvent("updateAssetData", {
                                    detail: data,
                                }),
                            );
                            pickFileModal.close();
                ...
                const observer = observeDom(modal, function (mutationList) {
                    [].forEach.call(mutationList, function (mutation) {
                        ...
                        [].forEach.call(mutation.addedNodes, function (node) {
                            addClickCallbacks(
                                node.querySelectorAll("div[data-json] > a"),
                            );
                        });
                    });
                });
