...
export default class JsonMenuNestedComponent {
    ...
    load({ activeItemId = null, loadChildrenId = null } = {}) {
        this._post("/render", {
            ...
            if (
                !Object.hasOwn(json, "tree") ||
                !Object.hasOwn(json, "load_parent_ids")
            )
                return;
    ...
    _addClickListeners() {
        this.element.addEventListener(
            ...
            (event) => {
                ...
                if (element.classList.contains("jmn-btn-add"))
                    this._onClickButtonAdd(element, itemId);
                if (element.classList.contains("jmn-btn-edit"))
                    this._onClickButtonEdit(element, itemId);
                if (element.classList.contains("jmn-btn-view"))
                    this._onClickButtonView(element, itemId);
                if (element.classList.contains("jmn-btn-delete"))
                    this._onClickButtonDelete(itemId);

                if (Object.hasOwn(element, "jmnModalCustom"))
    ...
    _onClickButtonAdd(element, itemId) {
        this._ajaxModal(
            element,
            `/item/${itemId}/modal-add/${element.dataset.add}`,
            "jmn-add",
        );
    }
    ...
    _onClickModalCustom(element, itemId) {
        ...
        this._ajaxModal(
            element,
            `/item/${itemId}/modal-custom/${modalCustomName}`,
            "jmn-modal-custom",
        );
    }
    ...
    _onClickButtonCollapse(button, longPressed = false) {
        ...
        if (expanded === "true") {
            ...
            const childIds = Array.from(childNodes).map(
    ...
    _onMove(event) {
        ...
        if (
            !Object.hasOwn(dragged.dataset, "type") ||
            !Object.hasOwn(targetList.dataset, "types")
        )
            return false;
    ...
    _onMoveEnd(event) {
        ...
        const targetComponent =
            window.jsonMenuNestedComponents[
                event.to.closest(".json-menu-nested-component").id
            ];
        const fromComponent =
            window.jsonMenuNestedComponents[
                event.from.closest(".json-menu-nested-component").id
            ];
        ...
        if (targetComponent.id === fromComponent.id) {
            ...
            fromComponent
                .itemGet(itemId)
                .then((json) => {
                    if (!Object.hasOwn(json, "item"))
                        throw new Error(JSON.stringify(json));
                    return targetComponent.itemAdd(
                        toParentId,
                        json.item,
                        position,
                    );
    ...
    _ajaxModal(element, path, eventType) {
        ...
        ajaxModal.load(
            { url: `${this.#pathPrefix}${path}`, size: modalSize },
            (json) => {
                ...
                if (eventType === "jmn-add" || eventType === "jmn-edit") {
                    if (!Object.hasOwn(json, "success") || !json.success)
                        return;
                    if (Object.hasOwn(json, "load"))
                        this.#loadParentIds.push(json.load);
                    if (
                        Object.hasOwn(json, "item") &&
                        Object.hasOwn(json, "id")
                    )
