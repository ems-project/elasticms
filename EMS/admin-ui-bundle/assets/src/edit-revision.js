        ...
        if (collectionName) {
            $(".collection-panel").each(function () {
                ...
                if (collectionPanel.data("name") === collectionName) {
                    const collectionLabelField = choice.data(
                    ...
                    collectionPanel
                        ...
                        .each(function () {
                            ...
                            if (expanded) {
                                const option = choice.find(
                                    'input[value="' + index + '"]',
                                );
                                if (option.length) {
                                    const parent =
                                        option.closest(".checkbox,.radio");
                                    if (
                                        $(
                                            "#" + id + "__ems_internal_deleted",
                                        ).val() === "deleted"
                                    ) {
                                        parent.hide();
                                        ...
                                        parent
                                            .find(".checkbox-radio-label-text")
                                            .text(label);
                                        parent.show();
                                    }
                                }
...
function onChange(allowAutoPublish = false) {
    ...
    waitingResponse = ajaxRequest
        .post(
            primaryBox.data("ajax-update"),
            $("form[name=revision]").serialize(),
        )
                    ...
                    switch (targetElement.nodeName) {
                        case "DIV": {
                            ...
                            if (
                                ...
                                const invalidFeedback = createInvalidFeedback(
                                    item.message,
                                );
                        ...
                        case "INPUT": {
                            ...
                            if (label !== null) {
                                ...
                                if (invalidFeedback !== null) {
                                    ...
                                    const invalidFeedback =
                                        createInvalidFeedback(item.message);
