...
export default class MediaLibrary {
    ...
    _init() {
        ...
        this._initInfiniteScrollFiles(
            this.#elements.files,
            this.#elements.loadMoreFiles,
        );

        Promise.allSettled([this._getFolders(), this._getFiles()]).then(() =>
            this.loading(false),
        );
    }
    ...
    loading(flag) {
        ...
        const uploadButton = this.#elements.inputUpload
            ? this.#elements.header.querySelector(
                  `label[for="${this.#elements.inputUpload.id}"]`,
              )
            : false;
    ...
    _addEventListeners() {
        ...
        this.element.onclick = (event) => {
            ...
            if (classList.contains("media-lib-file"))
                this._onClickFile(event.target, event);
            if (classList.contains("media-lib-folder"))
                this._onClickFolder(event.target);

            if (classList.contains("btn-file-upload"))
                this.#elements.inputUpload.click();
            if (classList.contains("btn-file-view"))
                this._onClickButtonFileView(event.target);
            if (classList.contains("btn-file-rename"))
                this._onClickButtonFileRename(event.target);
            if (classList.contains("btn-file-delete"))
                this._onClickButtonFileDelete(event.target);
            if (classList.contains("btn-files-delete"))
                this._onClickButtonFilesDelete(event.target);
            if (classList.contains("btn-files-move"))
                this._onClickButtonFilesMove(event.target);

            if (classList.contains("btn-folder-add"))
                this._onClickButtonFolderAdd();
            ...
            if (classList.contains("btn-home"))
                this._onClickButtonHome(event.target);
            if (classList.contains("breadcrumb-item"))
                this._onClickBreadcrumbItem(event.target);
            if (Object.hasOwn(event.target.dataset, "sortId"))
                this._onClickFileSort(event.target);
        ...
        ["dragenter", "dragover", "dragleave", "drop", "dragend"].forEach(
            (dragEvent) => {
                this.#elements.files.addEventListener(dragEvent, (event) =>
                    this._onDragUpload(event),
                );
            },
        );
    }
    ...
    _onClickFileSort(target) {
        ...
        if (Object.hasOwn(target.dataset, "sortOrder")) {
            this.#sortOrder =
                target.dataset.sortOrder === "asc" ? "desc" : "asc";
        }
    ...
    _onClickButtonFileView(button) {
        const getSiblingFile = (fileId, sibling) => {
            ...
            return rowSibling
                ? rowSibling.querySelector(".media-lib-file")
                : null;
        };

        const navigation = (action, sibling, fileId) => {
            const button = ajaxModal.modal.querySelector(
                `.btn-preview-${action}`,
            );
            ...
            button.addEventListener("click", () => {
                ...
                const header = this.#elements.files.querySelector(
                    ".media-lib-file-header",
                );
        ...
        const openModal = (fileId) => {
            const onClose = () => {
                ajaxModal.modal.removeEventListener(
                    "ajax-modal-close",
                    onClose,
                );
    ...
    _onClickButtonFileRename(button) {
        ...
        ajaxModal.load(
            ...
            (json) => {
                ...
                const { fileRow = "" } = json;
                if (fileRow.length > 0)
                    rowElement.closest("li").innerHTML = fileRow;
            ...
            (json) => {
                ...
                Promise.allSettled(
                    Array.from(selection).map((fileRow) => {
                        return this._post(
                            `/file/${fileRow.dataset.id}/delete`,
                        ).then(() => {
                            if (
                            ...
                            progressBar
                                .progress(
                                    Math.round(
                                        (++processed / selection.length) * 100,
                                    ),
                                )
                                .style("success");
                        });
    ...
    _onClickButtonFilesMove(button, targetId) {
        ...
        const path = this.#activeFolderId
            ? `/move-files/${this.#activeFolderId}`
            : "/move-files";
        const query = new URLSearchParams({
            selectionFiles: selection.length.toString(),
        });
        ...
        ajaxModal.load(
            ...
            (json) => {
                ...
                const progressBar = new ProgressBar("progress-move-files", {
                    label:
                        selection.length === 1 ? "Moving file" : "Moving files",
                    value: 100,
                    showPercentage: true,
                });
    ...
    _onClickButtonFolderAdd() {
        const path = this.#activeFolderId
            ? `/add-folder/${this.#activeFolderId}`
            : "/add-folder";
    ...
    _getLayout(fileId = null) {
        ...
        if (this.getSelectionFiles().length > 0)
            query.append(
                "selectionFiles",
                this.getSelectionFiles().length.toString(),
            );
        if (this.#activeFolderId)
            query.append("folderId", this.#activeFolderId);
        ...
        return this._get(path).then((json) => {
            ...
            if (Object.hasOwn(json, "footer"))
                this.#elements.footer.innerHTML = json.footer;
        });
    }
    ...
    _openPath(path) {
        ...
        path.split("/")
            ...
            .forEach((folderName) => {
                ...
                const parentLi = parentFolder
                    ? parentFolder.closest("li")
                    : null;
    ...
    _appendFiles(json) {
        ...
        if (rowHeader !== undefined) {
            ...
            if (Object.hasOwn(json, "sort"))
                this._displaySort(json.sort.id, json.sort.order);
        }
    ...
    _refreshHeader(html) {
        const searchBoxHasFocus =
            document.activeElement === this.getSearchBox();
    ...
    _onDragFolder(event) {
        ...
        if (event.type === "dragenter") {
            this.getFolders().forEach((f) =>
                f.classList.remove("media-lib-drop-area"),
            );
        ...
        if (event.type === "drop") {
            ...
            const moveButton =
                this.#elements.header.querySelector(".btn-files-move");
    ...
    async _resizeImage(file, fileHash) {
        return await resizeImage(
            this.#options.hashAlgo,
            this.#options.urlInitUpload,
            file,
        )
            .then((response) => {
                if (response === null) {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    this.loading(true);
                    this._getFiles(this.#loadedFiles).then(() =>
                        this.loading(false),
                    );
                }
            });
        ...
        if (!item.classList.contains("active")) {
            ...
            ["dragstart", "dragend"].forEach((dragEvent) => {
                if (!item._dragEventHandlers[dragEvent]) {
                    item._dragEventHandlers[dragEvent] = (event) =>
                        this._onDragFile(event);
                    item.addEventListener(
                        dragEvent,
                        item._dragEventHandlers[dragEvent],
                    );
                }
            });
        } else if (deselect) {
    ...
    _selectFiles(item, event) {
        if (event.shiftKey && this.#selectionLastFile !== null) {
            const files =
                this.#elements.listFiles.querySelectorAll(".media-lib-file");
    ...
    _selectFilesReset(refreshHeader = true) {
        if (refreshHeader === true)
            this._refreshHeader(this.#activeFolderHeader);
        this.getSelectionFiles().forEach((file) => {
            file.classList.remove("active");
            file.draggable = false;
            ["dragstart", "dragend"].forEach((dragEvent) => {
                file.removeEventListener(dragEvent, (event) =>
                    this._onDragFile(event),
                );
            });
        });
