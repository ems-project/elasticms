...
#[AsCommand(
    name: Commands::RUNNER_STATUS,
    description: 'Get the current status of a remote runner.',
    hidden: false
)]
class StatusCommand extends AbstractCommand
{
    ...
    {
        $status = $this->runnerManager->status($this->tag, $this->id);
        $this->io->title(\sprintf('Status of the runner "%s" started with id %s: %s', $this->tag, $this->id, $status->name));
