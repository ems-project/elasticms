...
- `http-options`: Allows overriding [S3 HTTP options](https://docs.aws.amazon.com/sdk-for-php/v3/developer-guide/guide_configuration.html#config-http), especially for `HEAD` requests. Default value: `{"connect_timeout":1.0,"timeout":5.0,"retries":0}`.
