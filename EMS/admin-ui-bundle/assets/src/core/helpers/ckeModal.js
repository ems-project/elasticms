...
export default class CkeModal {
    ...
    _treatResponse(response) {
        ...
        for (let i = 0; i < forms.length; ++i) {
            forms[i].addEventListener(EMS_FORM_RESPONSE_EVENT_EVENT, (event) =>
                self._onResponse(event),
            );
            forms[i].addEventListener(EMS_FORM_FAIL_EVENT_EVENT, (event) =>
                self._onFail(event),
            );
        }
    }
    ...
    _onResponse(event) {
        ...
        switch (event.detail.response.type) {
            ...
            case "edit-image": {
                const editImageEvent = new EditImageEvent(
                    event.detail.response.url,
                );
