...
function objectPickerListeners(objectPicker, maximumSelectionLength) {
    ...
    if (dynamicLoading) {
        const objectSearchUrl = document
            .querySelector("BODY")
            .getAttribute("data-search-api");
...
window.onload = function () {
    ...
    columnCriteria.change(function () {
        if (
            ...
            if ($(this).attr("id") === "criteria_filter_columnCriteria") {
                $("#criteria_filter_rowCriteria").val(
                    $("#criteria_filter_rowCriteria option:not(:selected)")
                        .first()
                        .val(),
                );
            } else {
                $("#criteria_filter_columnCriteria").val(
                    $("#criteria_filter_columnCriteria option:not(:selected)")
                        .first()
                        .val(),
                );
            }
        ...
        $("div#criterion select").each(function () {
            ...
            if (
                criterionName === colCriteria ||
                criterionName === rowCriteria
            ) {
