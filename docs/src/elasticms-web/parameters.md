...
By default, this option is set to `strict`. However, when using Single Sign-On ( SSO) with Keycloak
on a different domain, you need to change it to `lax` to ensure that cross-domain authentication
cookies work correctly. See the
[Symfony session configuration documentation](https://symfony.com/doc/current/session.html#cookie-samesite)
for more details.
...
Define a global firewall regex for forcing authentication, default value `null` The following
example protects all routes except if it starts with /api or /\_profiler, ... Example:
`^(?!(/api|/_profiler|/_wdt|/bundles|/favicon\.ico|/saml|/oauth2))`
...
for the form bundle. Set to `false` by default.
