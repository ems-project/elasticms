...
class File {
    ...
    load(target) {
        ...
        query
            .find(".file-uploader-row")
            .on("updateAssetData", function (event) {
        ...
        const fileUploaderInputs = target.getElementsByClassName(
            "file-uploader-input",
        );

        [].forEach.call(fileUploaderInputs, function (fileUploaderInput) {
            fileUploaderInput.onchange = () => {
                self.initFilesUploader(
                    fileUploaderInput.files,
                    fileUploaderInput,
                );
            };
        });
