    ...
    load(target) {
        ...
        [].forEach.call(datatables, function (element) {
            const datatable = $(element).DataTable(
                JSON.parse(element.dataset.datatable),
            );
            datatable.on("draw", () => Core.load(element));

            document
                .querySelectorAll(`[data-datatable-target='${element.id}']`)
                .forEach((btn) =>
                    btn.addEventListener("click", () => {
                        if (!Object.hasOwn(btn.dataset, "datatableEvent"))
                            return;
                        ...
                        element.dispatchEvent(
                            new CustomEvent(btn.dataset.datatableEvent, {
                                detail: {
                                    selection: Array.from(checked).map(
