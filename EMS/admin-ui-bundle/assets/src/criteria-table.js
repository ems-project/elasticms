...
function formatRepoSelectionForTable(repo) {
    ...
    if (color) {
        complementary =
            luma(color.replace("#", "")) >= 165 ? "#000000" : "#ffffff";
        style =
            ' style="color: ' +
            complementary +
            ";background-color: " +
...
window.onload = function () {
    ...
    selects.change(function () {
        const filters = $.extend(
            $(this).closest("td").data("filters"),
            $(this).closest("table").data("filters"),
        );
        ...
        $(this)
            ...
            .each(function () {
                ...
                if (optionElem.attr("data-status") !== "added") {
                    ...
                    ajaxRequest
                        .post(form.data("add-url"), data)
                        .success(function () {
                            optionElem.attr("data-status", "added");
                        })
                        .fail(function () {
                            optionElem.attr("data-status", "removed");
                            let valuesArray = optionElem
                                .parents("select")
                                .val();
                            valuesArray = $.grep(valuesArray, function (value) {
        ...
        $(this)
            ...
            .each(function () {
                ...
                if (optionElem.attr("data-status") !== "removed") {
                    ...
                    ajaxRequest
                        .post(form.data("remove-url"), data)
                        .success(function () {
                            optionElem.attr("data-status", "removed");
                        })
                        .fail(function () {
