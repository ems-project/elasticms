...
Routes can use the search controller, which using a predefined search config. The search config can
be defined in:
...
- minimum_should_match: the bool query's minimum should match parameter, default `1`. Check the
  [minimum_should_match elasticsearch documentation](https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-minimum-should-match.html).
            ...
            "should": [
                {
                    "multi_match": {
                        ...
                        "type": "bool_prefix",
                        "boost": 10,
                        "fields": ["title_%_locale%", "all_%_locale%"]
                    }
                },
                {
                    "query_string": {
...
By default post filtering is enable for public **terms** filters. This way the aggregations are
computed before filtering, we still known the counts of other choices.
...
Show all document bigger then now on the field new_until. Important we need to set the date_format
to false, because we are passing 'now'. If we let the user select the date from a datepicker we can
use the date_format and make the filter public.
...
Elasticms supports versioning on documents. Documents share a version uuid and have a from and to
date. The document without a to date is the current version.

This means we can search for date and get the matching version document. We will never have multiple
results for a certain date because elasticms does not create gabs between versions.
