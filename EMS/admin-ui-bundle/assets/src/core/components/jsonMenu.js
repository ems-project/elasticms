...
export default class JsonMenu {
    constructor(target) {
        ...
        this.nestedSortable = this.parent
            .find("ol.json_menu_sortable")
            .nestedSortable({
    ...
    addListeners(target) {
        ...
        jTarget
            .find(".json_menu_sortable_remove_button")
            .on("click", function (event) {
                self.removeElement(this, event);
            });
        jTarget
            .find(".json_menu_sortable_add_item_button")
            .on("click", function (event) {
                event.preventDefault();
                self.addItem($(this), "prototype-item", $(this).data());
            });
        jTarget
            .find(".json_menu_sortable_add_node_button")
            .on("click", function (event) {
                event.preventDefault();
                self.addItem($(this), "prototype-node", $(this).data());
            });
        jTarget
            .find(".json_menu_sortable_paste_button")
            .on("click", function (event) {
                event.preventDefault();
                self.paste($(this));
            });
    ...
    addItem($target, prototypeTarget, data) {
        const uuid = v4();
        let itemHtml = this.parent
            .find(".json_menu_editor_fieldtype_widget")
            .data(prototypeTarget);
    ...
    relocate() {
        ...
        const hierarchyValue = JSON.stringify(
            recursiveMapHierarchy(toHierarchy),
        );
